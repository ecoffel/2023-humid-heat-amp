    ...
    for x in range(day_of_max.lat.size):
        for y in range(day_of_max.lon.size):
            if ~pd.isnull(day_of_max[x,y]):
                val_on_max_day[x,y] = cmip6_huss_regrid[:,x,y].sel(time=day_of_max[x,y])
...
# if decile_var == 'tx':
if on_tx:
    output_file = f"output/cmip6/huss_on_txx_1981_2100_ssp245_{model}.nc"
else:
    output_file = f"output/cmip6/huss_on_tww_1981_2100_ssp245_{model}.nc"
