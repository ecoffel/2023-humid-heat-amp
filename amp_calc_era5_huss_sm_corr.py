...
file_path = '%s/daily/sm_%d.nc'%(dirEra5Land, year)
ds_sm_var = xr.open_dataset(file_path)


# Load the soil moisture dataset for the specified year
huss_file_path = '%s/daily/huss_%d.nc'%(dirEra5, year)
ds_huss_var = xr.open_dataset(huss_file_path)


# Check if the dimensions differ
if (ds_sm_var.longitude.size != ds_huss_var.longitude.size) or (ds_sm_var.latitude.size != ds_huss_var.latitude.size):

    ds_sm_var = ds_sm_var.rename({'latitude':'lat', 'longitude':'lon'})
    ds_huss_var = ds_huss_var.rename({'latitude':'lat', 'longitude':'lon'})
    
    # Define regridder
    regridder = xe.Regridder(ds_huss_var, ds_sm_var, 'bilinear', reuse_weights=True)

    # Apply regridding
    ds_huss_var = regridder(ds_huss_var)
    
    ds_sm_var = ds_sm_var.rename({'lat':'latitude', 'lon':'longitude'})
    ds_huss_var = ds_huss_var.rename({'lat':'latitude', 'lon':'longitude'})


# First create a boolean mask
mask = xr.full_like(ds_sm_var.time, False, dtype=bool)
...
for y in annual_max_months_da_tx.year:
    ...
    mask = mask | (ds_sm_var.time.dt.month == month_of_max_tx) | (ds_sm_var.time.dt.month == month_of_max_tw)

# Apply the mask to select temperature data for the months of interest
tx_months_of_interest = ds_sm_var['swvl1'].where(mask, drop=True)
huss_months_of_interest = ds_huss_var['q'].where(mask, drop=True)
    
corr = xr.corr(tx_months_of_interest, huss_months_of_interest, dim='time')

# Save the results to a netcdf file
output_file = f"output/huss_sm_corr/huss_sm_corr_warm_season_{year}.nc"
