...
tx_hist_time_mean = tx_hist[m].sel(time=slice('1980', '2010')).mean(dim='time')
tx_fut_time_mean = tx_fut[m].sel(time=slice('2020', '2050')).mean(dim='time')
...
tx_hist_ann_max = tx_hist[m].sel(time=slice('1980', '2010')).tasmax.resample(time='Y').max().mean(dim='time')
tx_fut_ann_max = tx_fut[m].sel(time=slice('2020', '2050')).tasmax.resample(time='Y').max().mean(dim='time')
...
cur_global_mean_tx_ts_fut = xgrid_utils.calc_spatial_mean(tx_fut[m].sel(time=slice('2020', '2050')).tasmax.resample(time='Y').max().rename({'lat':'latitude', 'lon':'longitude'}))
...
tw_hist_time_mean = tw_hist[m].sel(time=slice('1980', '2010')).mean(dim='time')
tw_fut_time_mean = tw_fut[m].sel(time=slice('2020', '2050')).mean(dim='time')
...
tw_hist_ann_max = tw_hist[m].sel(time=slice('1980', '2010')).tw.resample(time='Y').max().mean(dim='time')
tw_fut_ann_max = tw_fut[m].sel(time=slice('2020', '2050')).tw.resample(time='Y').max().mean(dim='time')
...
cur_global_mean_tw_ts_fut = xgrid_utils.calc_spatial_mean(tw_fut[m].sel(time=slice('2020', '2050')).tw.resample(time='Y').max().rename({'lat':'latitude', 'lon':'longitude'}))
