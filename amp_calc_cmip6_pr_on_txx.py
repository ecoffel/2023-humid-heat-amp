...
for y in annual_max_months_da_tx_regrid.year:
    ...
    mask = mask | (cmip6_lh_regrid.time.dt.month == month_of_max_tx) | (cmip6_lh_regrid.time.dt.month == month_of_max_tw)

# Apply the mask to select temperature data for the months of interest
cmip6_lh_regrid = cmip6_lh_regrid.where(mask, drop=True).mean(dim='time')
