...
era5_et_file_path = '%s/monthly/evapotranspiration_monthly_%d.nc'%(dirEra5Land, year)
ds_era5_et = xr.open_dataset(era5_et_file_path)


# Check if the dimensions differ
if (ds_temperature.longitude.size != ds_era5_et.longitude.size) or (ds_temperature.latitude.size != ds_era5_et.latitude.size):

    ds_temperature = ds_temperature.rename({'latitude':'lat', 'longitude':'lon'})
    ds_era5_et = ds_era5_et.rename({'latitude':'lat', 'longitude':'lon'})

    # Define regridder
    regridder = xe.Regridder(ds_era5_et, ds_temperature, 'bilinear', reuse_weights=True)

    # Apply regridding
    ds_era5_et = regridder(ds_era5_et)
    
    ds_temperature = ds_temperature.rename({'lat':'latitude', 'lon':'longitude'})
    ds_era5_et = ds_era5_et.rename({'lat':'latitude', 'lon':'longitude'})
...
for y in annual_max_months_da_tx.year:
    ...
    mask = mask | (ds_era5_et.time.dt.month == month_of_max_tx) | (ds_era5_et.time.dt.month == month_of_max_tw)

# Apply the mask to select temperature data for the months of interest
ds_era5_et = ds_era5_et[et_var].where(mask, drop=True).mean(dim='time')

# Save the results to a netcdf file
output_file = f"output/et_on_tx_season/et_on_tx_warm_season_{year}.nc"
ds_era5_et.to_netcdf(output_file)
